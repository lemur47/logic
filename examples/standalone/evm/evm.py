"""
EVM (Earned Value Management) - Standalone Version
...
Dependencies: None (matplotlib optional for visualize_progress)
...
Concepts:
    ...
    From these three inputs + BAC, everything else is computed.
...
allowed to diverge in STRUCTURE from `app/evm/core.py`, which is canonical and
...
Canonical implementation: app/evm/core.py
"""
...
def visualize_progress(result: dict, save_path: str | None = None):
    """Plot EVM S-curve showing PV, EV, AC against BAC.

    Args:
        result: Output from evaluate_progress().
        save_path: Optional file path. If None, displays interactively.
    ...
    # BAC reference line
    ax.axhline(
        y=inp["bac"], color="#6b7280", linestyle="--", linewidth=1, label=f"BAC = {inp['bac']:,.0f}"
    )
    ...
    # Status badge
    status_colors = {"on_track": "#22c55e", "at_risk": "#f59e0b", "off_track": "#ef4444"}
    status = health["status"]
    ax.text(
        0.98,
        0.95,
        f"SPI={metrics['spi']:.2f}  CPI={metrics['cpi']:.2f}\n{status.upper().replace('_', ' ')}",
        ...
        bbox={
            ...
            "facecolor": "white",
            "edgecolor": status_colors.get(status, "#6b7280"),
        },
    )

    ax.set_ylabel("Cost")
    ax.set_title(f"EVM Snapshot — {metrics['percent_complete']:.1f}% Complete")
    ax.legend(loc="upper left")
    ax.set_ylim(0, max(values + [inp["bac"]]) * 1.2)
...
def _demo():
    """Run a complete EVM scenario to demonstrate the module."""
    print("=" * 60)
    print("EVM (Earned Value Management) — Standalone Demo")
    print("=" * 60)
    ...
    print(f"BAC (Budget at Completion): ${baseline.bac:,.0f}")
    print(f"Work packages: {len(baseline.work_packages)}")
    for wp in baseline.work_packages:
        print(f"  {wp['name']}: ${wp['planned_value']:,.0f} (weight: {wp['weight']:.1%})")

    # --- 2. Mid-project checkpoint ---
    print("\n--- Step 2: Mid-Project Evaluation (Month 3 of 6) ---\n")
    ...
    print(f"Planned Value (PV):  ${inp['pv']:>10,.0f}  (what should be done)")
    print(f"Earned Value (EV):   ${inp['ev']:>10,.0f}  (what is done)")
    print(f"Actual Cost (AC):    ${inp['ac']:>10,.0f}  (what was spent)")
    print()
    print(f"Schedule Variance:   SV = ${m['sv']:>+10,.0f}  {'ahead' if m['sv'] >= 0 else 'behind'}")
    print(
        f"Schedule Perf Index: SPI = {m['spi']:>8.2f}    {'≥1.0 good' if m['spi'] >= 1.0 else '<1.0 behind'}"
    )
    print(
        f"Cost Variance:       CV = ${m['cv']:>+10,.0f}  {'under' if m['cv'] >= 0 else 'over'} budget"
    )
    print(
        f"Cost Perf Index:     CPI = {m['cpi']:>8.2f}    {'≥1.0 good' if m['cpi'] >= 1.0 else '<1.0 over budget'}"
    )
    print()
    print(f"Estimate at Compl:   EAC = ${m['eac']:>10,.0f}  (projected total cost)")
    print(f"Estimate to Compl:   ETC = ${m['etc']:>10,.0f}  (remaining cost)")
    print(f"Variance at Compl:   VAC = ${m['vac']:>+10,.0f}  (projected overrun)")
    print(f"To-Compl Perf Index: TCPI = {m['tcpi']:>7.2f}    (required CPI for remaining work)")
    print()
    print(
        f"Progress: {m['percent_complete']:.1f}% complete, {m['percent_spent']:.1f}% of budget spent"
    )
    print()
    print(f"Health: {h['status'].upper().replace('_', ' ')}")
    print(f"Summary: {h['summary']}")
    for reason in h["reasons"]:
        print(f"  → {reason}")

    # --- 3. Work package breakdown ---
    print("\n--- Step 3: Work Package Breakdown ---\n")

    for wp in result["work_packages"]:
        bar_len = int(wp["percent_complete"] / 5)
        bar = "█" * bar_len + "░" * (20 - bar_len)
        print(
            f"  {wp['name']:<28s} {bar} {wp['percent_complete']:>5.0f}%  "
            f"EV: ${wp['earned_value']:>8,.0f} / ${wp['planned_value']:>8,.0f}"
        )

    # --- 4. Direct metrics call ---
    print("\n--- Step 4: Direct evm_metrics() Call ---\n")

    raw = evm_metrics(pv=100_000, ev=85_000, ac=95_000, bac=200_000)
    print(f"  SPI={raw['spi']:.2f}  CPI={raw['cpi']:.2f}  EAC=${raw['eac']:,.0f}")

    signal = health_signal(raw["spi"], raw["cpi"])
    print(f"  Status: {signal['status']}  — {signal['summary']}")

    # --- 5. Custom thresholds ---
    print("\n--- Step 5: Custom Thresholds (strict startup) ---\n")
    ...
    strict_signal = health_signal(spi=0.92, cpi=0.97, thresholds=strict)
    print("  SPI=0.92, CPI=0.97 with strict thresholds:")
    print(f"  Status: {strict_signal['status']}")
    for r in strict_signal["reasons"]:
        print(f"    → {r}")

    print("\n" + "=" * 60)
    print("Done. All EVM functions are importable:")
    print("  from evm import evm_metrics, create_baseline, evaluate_progress")
    print("=" * 60)
