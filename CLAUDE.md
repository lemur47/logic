...
- **Silence is narrower than it was, and still ambiguous.** This line previously said a GitHub `406` on the diff fetch produced no comment and no usage row. **That path is now handled**: the diff fetch has an error branch that comments on the pull request and records the run. Proved by forcing the failure against a real webhook delivery, not by reading the config. **But three modules still stop with no handler** — the model call, the comment post and the PR-metadata fetch — and `dlq` is off, so those runs are lost rather than parked for a manual resume. Left that way deliberately: the auditor is repository automation, not the product, and a missing review comment is not an incident. **Still do not read a missing comment as a clean review.**
...
- **The user turn is guarded too, since 2026-08-27.** It was not, and that is where the delimiters live — the pairing drifted twice inside a single pull request, and `reviewer-prompt.md` documented a turn that had not shipped for weeks. `tests/test_pr_auditor_prompt_parity.py` now asserts its **shape**, not its bytes: both markers present and ordered, the 200,000-character cap self-consistent across its two uses, the file-header count in its exact anchored form, no PR title or body, and every `{{1.*}}` field in the operator-authored half drawn from an allowlist. The cut-off notice is asserted against **module 7**, where it lives. Shape rather than bytes because the document describes the turn with placeholders where the deployed turn carries expressions; a byte comparison could only pass by making the document useless to a human.
...
- **No local hook sees the message that actually lands on `main`.** This repository squash-merges, so the merge commit is composed in the GitHub web interface — as are web-editor commits, and any commit from a clone where `pre-commit install` was never run. The required `gitleaks` job therefore also runs `check-airtable-ids.py --pr-text` over the pull request's title, body and commit messages, re-running on `edited` as well as on push so a later title change cannot keep an earlier green. **It does not reach a message rewritten in the squash dialog** — that edit re-triggers no check, and no required context can see it. It is a **step inside that job, not a job of its own**, because "Protect main" lists nine contexts by name and is managed outside this repository: a new job would be green-but-not-blocking until someone edited the ruleset by hand.
...
- **A deferral carries an expiry, and the expiry is enforced rather than written down.** Three surfaces hold dated deferrals: the npm `overrides` in `site/package.json`, the `ignore:` blocks in `.github/dependabot.yml`, and `osv-scanner.toml` whenever a waiver exists. `tests/test_dependency_overrides.py` requires each one to carry a date — `review-by YYYY-MM-DD` for an `ignore:`, a re-check date for an override — in the comment directly above it, and reds the required `pytest` job once that date passes. **Deliberate consequence: an arriving date reds a green tree with no commit behind it**, the same shape as the weekly scheduled scan. Re-checking is not the same as lifting the hold: confirm whether the constraint still stands, write down what you found, then move the date. **Never extend a date silently.** ⚠️ **The test asserts that a reason EXISTS, not that it is TRUE** — the `undici` override's stated reason went false one day after it was written, and nothing went red.
- **Two known holes in that check, named rather than quietly patched.** The `ignore:` scan matches the exact stripped line `ignore:`, so a flow-style `ignore: [{...}]` is skipped **silently** and an undated hold would pass — the fail-by-silence class, and the reason to keep writing these blocks in block style. Separately, the comment block must sit immediately above the key with no blank line between; that one fails **loudly**, which is the right direction.
...
- **A stale `.git/config.lock` makes git fail silently, and Claude Code's own sandbox creates it.** Bubblewrap masks that path by bind-mounting `/dev/null` over it; to have a mount point it first creates the file — 0 bytes, mode `444` — and does **not** remove it when the sandbox exits. Inside the sandbox the path stats as a character device (`crw-rw-rw- … 1, 3`); from a normal shell it is an ordinary empty read-only file. It comes back after essentially any sandboxed command in the repo, so treat its reappearance as expected rather than as a new fault. **Run `rm -f .git/config.lock` before any git operation that writes config** — `branch -d`, `push -u`, `remote`, `config`.
- **A git warning that does not change the exit code is a failure.** With that lock present, `git branch -d` deletes the ref and orphans its `[branch]` section, `git push -u` pushes the branch and never records the upstream, and `git worktree remove` leaves `commondir` and `config.worktree` behind as 0-byte 444 residue — **all printing `could not lock config file` and all exiting 0.** Read stderr, not just the status. Repair with `git branch --set-upstream-to=origin/<branch> <branch>` and `git config --unset-all branch.<branch>.remote`. Same class as the dead gitleaks hook: the thing reports success and did nothing.
- This cost five weeks and three false theories — a compound-command theory, a serial-writes rule, and an inode-scoped `auditd` watch that was armed against a daemon which **was not running**. It was settled in minutes by `fatrace` naming the process, after the watcher itself was proved with a canary. **When something is creating or deleting a file you cannot account for, watch the filesystem before reasoning about who might be responsible.**
