...
description: "As more of delivery is handed to AI, what boards must govern shifts from the deliverable to the mechanism producing it. But mechanisms do not break by stopping. They stay broken and keep reporting success — we found four in our own delivery in a single week. Here are the four points worth verifying."
pubDate: 2026-08-01
...
The difficulty is that the mechanism has its own trapdoors. It does not necessarily break by stopping. It stays broken and keeps reporting success. We found four instances in our own delivery in a single week — in an organisation whose business is building assurance tooling.
...
- None of this is peculiar to AI. "Reports clean, work never done" recurred throughout offshore development and multi-vendor governance, and it is continuous with the deference PMOs already know from estimates, risk analysis and completion reports.
...
## Analysis: A Self-Running Mechanism Does Not Break by Stopping — It Stays Broken and Keeps Reporting Success

![Diagram of the four verification points — a control that ran and found nothing and a control that never ran, or watched the wrong target, produce the same "nothing found" output; four questions tell them apart](/blog/verifying-self-running-systems/verification-points.svg)
...
In the same week, a procedure verifying that a set of files had copied correctly reported a clean pass on both of its checks. It had been verifying a location that did not exist, because an environment variable resolved to different values in two steps. An absent target produced the same output as a normal result. Re-run against the right path it genuinely passed — but the first report certified nothing while stating it had certified everything.
...
![Diagram of intervention points in the self-running loop — plan and instruct, agent executes, control runs, report and decide; each of the four verification points intervenes at a specific place, inside the environment-and-structure layer rather than the prompt layer](/blog/verifying-self-running-systems/loop-intervention-points.svg)
